...
import json                      # settings.json okuma/yazma
...
from PyQt5.QtWidgets import (
    ...
    QStackedWidget, QComboBox, QSpinBox, QDialog, QPlainTextEdit,
)
...
I18N = {
    'tr': {
        'window_title': 'SEGESYS // vm.forge — SegeSystems Sanal PC Kurulum Aracı v1.2',
        ...
        # ── v1.2: base dizini + bypass editör ──
        'base_section': 'BASE DİZİNİ',
        'base_placeholder': 'Documents/Virtual Machines (varsayılan)',
        'base_browse_title': 'VM kayıt dizinini seçin',
        'base_log': 'base dizini → {path}',
        'base_invalid': 'Base dizini geçersiz veya yazılabilir değil!',
        'edit': '⚙',
        'bypass_editor_title': 'Bypass Anahtarlarını Düzenle',
        'bypass_editor_hint': 'Her satır bir VMX anahtarı. Yanlış sözdizimi VM\'i bozabilir.',
        'bypass_default_btn': 'VARSAYILANA DÖN',
        'bypass_save_btn': 'KAYDET',
        'cancel_btn': 'İPTAL',
        'bypass_loaded': 'bypass.txt yüklendi · {n} satır',
        'bypass_saved': 'bypass.txt kaydedildi · {n} satır',
        'bypass_reset_log': 'bypass varsayılana döndü',
    },
    'en': {
        'window_title': 'SEGESYS // vm.forge — SegeSystems Virtual PC Setup Tool v1.2',
        ...
        # ── v1.2: base directory + bypass editor ──
        'base_section': 'BASE FOLDER',
        'base_placeholder': 'Documents/Virtual Machines (default)',
        'base_browse_title': 'Select VM storage folder',
        'base_log': 'base folder → {path}',
        'base_invalid': 'Base folder invalid or not writable!',
        'edit': '⚙',
        'bypass_editor_title': 'Edit Bypass Keys',
        'bypass_editor_hint': 'Each line is a VMX key. Bad syntax may break the VM.',
        'bypass_default_btn': 'RESTORE DEFAULTS',
        'bypass_save_btn': 'SAVE',
        'cancel_btn': 'CANCEL',
        'bypass_loaded': 'bypass.txt loaded · {n} lines',
        'bypass_saved': 'bypass.txt saved · {n} lines',
        'bypass_reset_log': 'bypass restored to defaults',
    },
}
...
# ════════════════════════════════════════════════════════════════════
# BYPASS ANAHTARLARI DÜZENLEYİCİ DİYALOĞU (v1.2)
# ════════════════════════════════════════════════════════════════════
# Kullanıcı kaynak koda dokunmadan bypass anahtarlarını UI'dan
# düzenleyebilsin diye eklendi. Düzenlenmiş içerik script'in yanındaki
# `bypass.txt` dosyasına kaydedilir; var oldukça program bunu kullanır,
# yoksa sınıf içindeki `_DEFAULT_BYPASS_KEYS` sabitine düşer.
class BypassEditDialog(QDialog):
    """Bypass anahtarlarını düzenleme penceresi."""

    def __init__(self, parent, current_text: str, default_text: str,
                 lang_dict: dict, mono_family: str):
        super().__init__(parent)
        self._default = default_text
        self._lang = lang_dict

        self.setWindowTitle(self._lang.get('bypass_editor_title',
                                           'Edit Bypass Keys'))
        ...
        # Açıklama satırı
        hint = QLabel(self._lang.get(
            'bypass_editor_hint',
            'Each line is a VMX key. Bad syntax may break the VM.',
        ))
        hint.setStyleSheet(f"color: {C['dim']}; font-size: 11px;")
        hint.setWordWrap(True)
        v.addWidget(hint)

        # Editör — tüm bypass anahtarları tek metin alanında
        self.editor = QPlainTextEdit()
        self.editor.setStyleSheet(
            f"background-color: #050709; color: {C['text']}; "
            f"border: 1px solid {C['line']}; border-radius: 6px; "
            f"padding: 8px; font-family: {mono_family}; font-size: 11px;"
        )
        self.editor.setPlainText(current_text)
        v.addWidget(self.editor, 1)

        # Alt buton barı: VARSAYILANA DÖN | İPTAL | KAYDET
        btn_row = QHBoxLayout()
        btn_row.setSpacing(8)

        self.reset_btn = QPushButton(self._lang.get('bypass_default_btn',
                                                    'RESTORE DEFAULTS'))
        self.reset_btn.setProperty('class', 'tab')
        ...
        self.cancel_btn = QPushButton(self._lang.get('cancel_btn', 'CANCEL'))
        self.cancel_btn.setProperty('class', 'tab')
        self.cancel_btn.setFixedHeight(32)
        self.cancel_btn.setCursor(Qt.PointingHandCursor)
        self.cancel_btn.clicked.connect(self.reject)
        btn_row.addWidget(self.cancel_btn)

        self.save_btn = QPushButton(self._lang.get('bypass_save_btn', 'SAVE'))
        self.save_btn.setObjectName('executeBtn')  # neon yeşil
    ...
    def _reset(self):
        """Editöre varsayılan bypass anahtarlarını yükle."""
        self.editor.setPlainText(self._default)

    def text(self) -> str:
        """Düzenleyicideki güncel metni döndür."""
...
class SegeSystemsGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # ── Persistent ayarlar (settings.json'dan yükle) ──────────────
        self._settings = self._load_settings()
        # Base dizini: kullanıcı seçimi varsa onu, yoksa Documents/Virtual Machines
        self.base_dir = self._settings.get(
            'base_dir',
            os.path.join(os.path.expanduser('~'), 'Documents', 'Virtual Machines'),
        )
        # Dil: settings'ten override (geriye uyumlu — yoksa 'tr' kalır)
        self.lang = self._settings.get('lang', 'tr')

        # ── Bypass anahtarları (kullanıcı düzenlemiş olabilir) ────────
        # bypass.txt programın yanında varsa onu kullan; yoksa default
        self._bypass_keys = self._load_bypass_keys()
        ...
        # v1.2: kullanıcı bypass.txt veya özel base dir kullanıyorsa duyur
        if self._bypass_path().is_file():
            n = len([l for l in self._bypass_keys.splitlines() if l.strip()])
            self._push_log(self.t('bypass_loaded', n=n), 'sys')
        if self._settings.get('base_dir'):
            self._push_log(self.t('base_log', path=self.base_dir), 'sys')

    # ═══ Persistent ayarlar (settings.json + bypass.txt) ═════════════
    @staticmethod
    def _settings_path() -> Path:
        """Script'in yanındaki settings.json'un yolu."""
        return script_dir() / 'settings.json'

    @staticmethod
    def _bypass_path() -> Path:
        """Script'in yanındaki bypass.txt'nin yolu (varsa kullanıcı override)."""
        return script_dir() / 'bypass.txt'

    def _load_settings(self) -> dict:
        """settings.json'dan ayarları yükle. Hata olursa boş dict döner."""
    ...
    def _save_settings(self):
        """Mevcut ayarları diske yaz (base_dir, lang)."""
        try:
            data = {'base_dir': self.base_dir, 'lang': self.lang}
            self._settings_path().write_text(
                json.dumps(data, indent=2, ensure_ascii=False),
                encoding='utf-8',
            )
        except OSError as e:
            self.signals.log.emit(f'settings save fail · {e}', 'err')

    def _load_bypass_keys(self) -> str:
        """bypass.txt varsa içeriğini, yoksa default sabiti döndür."""
        path = self._bypass_path()
        if not path.is_file():
            return self._DEFAULT_BYPASS_KEYS
        try:
            return path.read_text(encoding='utf-8')
        except OSError:
            return self._DEFAULT_BYPASS_KEYS

    def _save_bypass_keys(self, text: str):
        """Düzenlenmiş bypass anahtarlarını bypass.txt'e yaz."""
        try:
            self._bypass_path().write_text(text, encoding='utf-8')
        except OSError as e:
            self.signals.log.emit(f'bypass.txt save fail · {e}', 'err')
    ...
    def _set_lang(self, lang: str):
        """Dili değiştir, tüm metinleri tazele, settings'e kaydet."""
        if lang == self.lang:
            return
        self.lang = lang
        ...
        self._save_settings()  # v1.2: kalıcı tut
    ...
    def _refresh_texts(self):
        ...
        self.base_section.setText(self.t('base_section'))
        self.base_entry.setPlaceholderText(self.t('base_placeholder'))
        self.bypass_edit_btn.setToolTip(self.t('bypass_editor_title'))
    ...
    def _build_left_panel(self) -> QFrame:
        ...
        # ── Base dizini (v1.2) ─────────────────────────────────────
        # VM klasörlerinin oluşturulacağı kök dizin. Kullanıcı D:, E:
        # gibi farklı sürücüye yönlendirebilir; settings.json'da saklanır.
        self.base_section = self._section_label(self.t('base_section'), C['cyan'])
        v.addWidget(self.base_section)
        base_row = QHBoxLayout()
        self.base_entry = QLineEdit(self.base_dir)
        self.base_entry.setPlaceholderText(self.t('base_placeholder'))
        self.base_entry.editingFinished.connect(self._on_base_entry_edited)
        base_row.addWidget(self.base_entry)
        self.base_browse_btn = QPushButton('📁')
        self.base_browse_btn.setProperty('class', 'select')
        self.base_browse_btn.setFixedSize(40, 32)
        self.base_browse_btn.setCursor(Qt.PointingHandCursor)
        self.base_browse_btn.clicked.connect(self.select_base_dir)
        base_row.addWidget(self.base_browse_btn)
        ...
        # BYPASS ET + ⚙ düzenle butonu yan yana
        bypass_row = QHBoxLayout()
        bypass_row.setSpacing(6)
        ...
        bypass_row.addWidget(self.bypass_btn, 1)
        # v1.2: bypass anahtarlarını UI'dan düzenleme butonu
        self.bypass_edit_btn = QPushButton(self.t('edit'))
        self.bypass_edit_btn.setObjectName('bypassBtn')
        self.bypass_edit_btn.setFixedSize(40, 40)
        self.bypass_edit_btn.setCursor(Qt.PointingHandCursor)
        self.bypass_edit_btn.setToolTip(self.t('bypass_editor_title'))
        self.bypass_edit_btn.clicked.connect(self.open_bypass_editor)
        bypass_row.addWidget(self.bypass_edit_btn)
        v.addLayout(bypass_row)
    ...
    # ── v1.2: Base dizini seçimi ────────────────────────────────────
    def select_base_dir(self):
        """Klasör seçici aç → VM base dizinini güncelle ve settings'e yaz."""
        start = self.base_entry.text() or os.path.expanduser('~')
        path = QFileDialog.getExistingDirectory(
            self, self.t('base_browse_title'), start,
        )
        if path:
            self._apply_base_dir(path)

    def _on_base_entry_edited(self):
        """LineEdit'te elle yol değiştirildiğinde tetiklenir."""
        path = self.base_entry.text().strip()
        if path:
            self._apply_base_dir(path)

    def _apply_base_dir(self, path: str):
        """Base dizinini doğrula, uygula ve kalıcı tut."""
        # Yol mevcut değilse oluşturmayı dene
        try:
            os.makedirs(path, exist_ok=True)
        except OSError:
            self._error(self.t('base_invalid'))
            return
        # Yazma testi
        if not os.access(path, os.W_OK):
            ...
            return
        self.base_dir = path
        self.base_entry.setText(path)
        self._push_log(self.t('base_log', path=path), 'sys')
        self._save_settings()

    # ── v1.2: Bypass anahtarları düzenleyici ─────────────────────────
    def open_bypass_editor(self):
        """Bypass düzenleme dialogunu aç; KAYDET'e basılırsa diske yaz."""
        # Mono font'u app stylesheet'inden al — düzenleyici de aynı görünsün
        mono = _pick_mono_font()
        dialog = BypassEditDialog(
            parent=self,
            current_text=self._bypass_keys.lstrip('\n'),
            default_text=self._DEFAULT_BYPASS_KEYS.lstrip('\n'),
            lang_dict=I18N[self.lang],
            mono_family=mono,
        )
        if dialog.exec_() != QDialog.Accepted:
            return
        new_text = dialog.text().strip('\n')
        # Default ile aynıysa bypass.txt'i sil (geri varsayılana dön)
        default_clean = self._DEFAULT_BYPASS_KEYS.strip('\n')
        if new_text == default_clean:
            try:
                self._bypass_path().unlink(missing_ok=True)
            except OSError:
                pass
            self._bypass_keys = self._DEFAULT_BYPASS_KEYS
            self._push_log(self.t('bypass_reset_log'), 'sys')
        else:
            # Bypass anahtarları '\n' prefix'i ile başlamalı (mevcut formatla uyum)
            self._bypass_keys = '\n' + new_text
            self._save_bypass_keys(new_text)
            n_lines = len([l for l in new_text.splitlines() if l.strip()])
            self._push_log(self.t('bypass_saved', n=n_lines), 'sys')
    ...
    def _installation_thread(self, plan: list):
        ...
        try:
            ...
            # v1.2: kullanıcı tarafından seçilmiş base dizini (settings.json)
            vm_root = self.base_dir
    ...
    # ── VMware Anti-Detection Anahtarları (Default) ─────────────────
    # Bypass tıklanınca her .vmx dosyasının sonuna bu blok eklenir.
    # v1.2'den itibaren kullanıcı UI'dan ⚙ butonuyla bu listeyi
    # düzenleyebilir; düzenlenmiş içerik bypass.txt'e kaydedilir ve
    # bu sabitin yerine kullanılır. Reset butonu ile bu sabite geri döner.
    # Detaylı açıklamalar için docs/bypass.md.
    # Özet:
    ...
    #   - scsi0:0.{productID,vendorID} → SCSI disk model spoof (v1.1'den itibaren etkili)
    _DEFAULT_BYPASS_KEYS = (
    ...
    def _bypass_thread(self, target_prefix: str):
        ...
        """
        try:
            # v1.2: kullanıcı tarafından seçilmiş base dizini
            vm_root = self.base_dir
            ...
            for i, vm_name in enumerate(vm_dirs):
                ...
                try:
                    ...
                    if 'hypervisor.cpuid.v0' not in content:
                        # v1.2: instance üzerinden — kullanıcı düzenlemiş olabilir
                        content += self._bypass_keys
